...
struct PlannerState: Codable {
    ...
    var icmBaseline: [Double]? = nil
    var altitudeTrip: AltitudeTrip? = nil
    var altitudeSettings: AltitudeSettings? = nil
}
...
final class PlannerModel: ObservableObject {
    ...
    @Published var icmBaseline: [Double]? = nil
    @Published var lastProfile: String? = nil
    @Published var lastTissue: String? = nil
    @Published var lastICM: [Double]? = nil
    @Published var altitudeTrip = AltitudeTrip()
    @Published var altitudeSettings = AltitudeSettings()
    ...
    private func apply(_ s: PlannerState) {
        ...
        altitudeTrip = s.altitudeTrip ?? AltitudeTrip()
        altitudeSettings = s.altitudeSettings ?? AltitudeSettings()
    }
    ...
    private var snapshot: PlannerState {
        ...
        s.altitudeTrip = altitudeTrip
        s.altitudeSettings = altitudeSettings
        return s
    }
    ...
    var surfaceIntervalMinutes: Double {
        let f = surfaceInterval.split(separator: ":").compactMap { Double($0) }
    ...
    func calculate() {
        ...
        do {
            ...
            if hasResidual && icmBaseline == nil {
                lastICM = nil
            } else {
                lastICM = try? ZPlan.interconnectedState(profile: profileText, tissueFile: baselineTissue,
                                                         previous: hasResidual ? icmBaseline : nil,
...
public struct ZPlannerView: View {
    ...
    public var body: some View {
        ...
        .sheet(isPresented: $showAltitude) { AltitudeSheet(m: m) }
    ...
    /// e.g. "1.1 (7) · engine 1.10.0 · AI-(Lalo)assisted" — what a tester should quote in a report.
    private var buildStamp: String {
        let info = Bundle.main.infoDictionary
        let short = info?["CFBundleShortVersionString"] as? String ?? "?"
        let build = info?["CFBundleVersion"] as? String ?? "?"
        return "\(short) (\(build)) · engine \(ZPlan.version) · AI-(Lalo)assisted"
    }
    ...
    private var siRow: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 12) {
                if m.lastProfile != nil {
                    Button { showAltitude = true } label: {
                        Text("AAD Calc").font(.callout)
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    group("Altitude after diving") {
                        Text("The AAD Calc button, beside Surface Interval, appears once a dive is calculated. It tells you how long to wait before driving or flying to altitude after that dive or series, and what surface oxygen changes. Method 1 is Bühlmann: the gradient factor the trip needs, against a limit. Method 2 is the Di Muro interconnected model: the P(DCS) the trip adds. Neither model was fitted to altitude exposures.")
                            .font(.caption).foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                        row("Method 1 limit") {
                            Picker("", selection: $m.altitudeSettings.method1Limit) {
                                Text("DAN 12 h").tag(AltitudeMethod1Limit.danAnchored)
                                Text("Dive GF High").tag(AltitudeMethod1Limit.diveGFHigh)
                            }.pickerStyle(.segmented).labelsHidden()
                        }
                        row("Method 2 model") {
                            Picker("", selection: $m.altitudeSettings.method2Model) {
                                Text("UT").tag(AltitudeMethod2Model.ut)
                                Text("EE1").tag(AltitudeMethod2Model.ee1)
                            }.pickerStyle(.segmented).labelsHidden()
                        }
                        row("Method 2 limit") {
                            Picker("", selection: $m.altitudeSettings.method2Total) {
                                Text("Added by trip").tag(false)
                                Text("Total").tag(true)
                            }.pickerStyle(.segmented).labelsHidden()
                        }
                        HStack(spacing: 16) {
                            PercentField(label: "P(DCS) limit (%)",
                                         value: m.altitudeSettings.method2LimitPercent) {
                                m.altitudeSettings.method2LimitPercent = min(max($0, 0.1), 20)
                            }
                            PercentField(label: "Mask O2 (%)",
                                         value: m.altitudeSettings.maskO2Percent) { v in
                                m.altitudeSettings.maskO2Percent = min(max(v <= 1 ? v * 100 : v, 21), 100)
                            }
                        }
                        Text("Mask O2: 100 for a demand valve with a sealed oronasal mask or a mouthpiece and nose clip; 60 to 90 for a non-rebreather mask at 15 L/min. P(DCS) limit in percent, decimals allowed: 2.5 means 2.5%.")
                            .font(.caption).foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                }
